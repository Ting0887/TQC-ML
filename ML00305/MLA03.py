...
def measurement(model ,X_train, X_test):
    ...
    adj_r2_train = round(adj_R2(model.score(X_train, y_train), X_train.shape[0], X_train.shape[1]), 4)
    r2_test = round(model.score(X_test, y_test), 4)
    adj_r2_test = round(adj_R2(model.score(X_test, y_test), X_test.shape[0], X_test.shape[1]), 4)
...
lst_info.append(['複回歸','15 features'])
...
lst_info.append(['多項式迴歸 + Lasso迴歸','deg=2'])
...
if idx <= 1:
    ...
    print('房價預測結果: %d'%model.predict(df_poly_feat))
